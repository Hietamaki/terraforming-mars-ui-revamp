<template>
  <div id="player-home" :class="(game.turmoil ? 'with-turmoil' : '')">
    ...
    <sidebar v-trim-whitespace :acting_player="isPlayerActing(playerView)" :player_color="thisPlayer.color"
      :generation="game.generation" :coloniesCount="game.colonies.length" :temperature="game.temperature"
      :oxygen="game.oxygenLevel" :oceans="game.oceans" :venus="game.venusScaleLevel" :turmoil="game.turmoil"
      :moonData="game.moon" :gameOptions="game.gameOptions" :playerNumber="playerView.players.length"
    ...
    <div v-if="thisPlayer.tableau.length > 0">
      <div class="player_home_block">
        ...
        <board :spaces="game.spaces" :venusNextExtension="game.gameOptions.venusNextExtension"
          :venusScaleLevel="game.venusScaleLevel" :boardName="game.gameOptions.boardName" :oceans_count="game.oceans"
          :oxygen_level="game.oxygenLevel" :temperature="game.temperature"
          :aresExtension="game.gameOptions.aresExtension" :pathfindersExpansion="game.gameOptions.pathfindersExpansion"
          :altVenusBoard="game.gameOptions.altVenusBoard" :aresData="game.aresData" :tileView="tileView"
          @toggleTileView="cycleTileView()" id="shortkey-board" />
        ...
        <MoonBoard v-if="game.gameOptions.moonExpansion" :model="game.moon" :tileView="tileView"
          id="shortkey-moonBoard" />

        <PlanetaryTracks v-if="game.gameOptions.pathfindersExpansion" :tracks="game.pathfinders"
      ...
      <players-overview class="player_home_block player_home_block--players nofloat" :playerView="playerView"
        v-trim-whitespace id="shortkey-playersoverview" />

      <div class="player_home_block nofloat">
        <log-panel :id="playerView.id" :players="playerView.players" :generation="game.generation"
          :lastSoloGeneration="game.lastSoloGeneration" :color="thisPlayer.color" :step="game.step"></log-panel>
      </div>

      <div class="player_home_block player_home_block--actions nofloat">
        <a name="actions" class="player_home_anchor"></a>
        <dynamic-title title="Actions" :color="thisPlayer.color" />
        <waiting-for v-if="game.phase !== 'end'" :players="playerView.players" :playerView="playerView"
          :settings="settings" :waitingfor="playerView.waitingFor"></waiting-for>
      </div>
      ...
      <div class="player_home_block player_home_block--hand" v-if="cardsInHandCount > 0" id="shortkey-hand">
        <dynamic-title title="Cards In Hand" :color="thisPlayer.color" :withAdditional="true"
          :additional="cardsInHandCount.toString()" />
        <sortable-cards :playerId="playerView.id"
      ...
      <div class="player_home_block player_home_block--cards">
          ...
          <div class="played-cards-filters">
            <div :class="getHideButtonClass('ACTIVE')" v-on:click.prevent="toggle('ACTIVE')">
              <div class="played-cards-count">{{
                getCardsByType(thisPlayer.tableau,
                [CardType.ACTIVE]).length.toString()
              }}</div>
            ...
            <div :class="getHideButtonClass('AUTOMATED')" v-on:click.prevent="toggle('AUTOMATED')">
              <div class="played-cards-count">{{
                getCardsByType(thisPlayer.tableau, [CardType.AUTOMATED,
                  CardType.PRELUDE]).length.toString()
              }}</div>
            ...
            <div :class="getHideButtonClass('EVENT')" v-on:click.prevent="toggle('EVENT')">
              <div class="played-cards-count">{{ getCardsByType(thisPlayer.tableau, [CardType.EVENT]).length.toString() }}
              </div>
        ...
        <div v-for="card in getCardsByType(thisPlayer.tableau, [CardType.CORPORATION])" :key="card.name"
          class="cardbox">
        ...
        <div v-show="isVisible('ACTIVE')"
          v-for="card in sortActiveCards(getCardsByType(thisPlayer.tableau, [CardType.ACTIVE]))" :key="card.name"
          class="cardbox">
          <Card :card="card" :actionUsed="isCardActivated(card, thisPlayer)" />
        </div>

        <stacked-cards v-show="isVisible('AUTOMATED')"
          :cards="getCardsByType(thisPlayer.tableau, [CardType.AUTOMATED, CardType.PRELUDE])"></stacked-cards>

        <stacked-cards v-show="isVisible('EVENT')"
          :cards="getCardsByType(thisPlayer.tableau, [CardType.EVENT])"></stacked-cards>
      ...
      <div v-if="thisPlayer.selfReplicatingRobotsCards.length > 0" class="player_home_block">
        ...
        <div>
          <div v-for="card in getCardsByType(thisPlayer.selfReplicatingRobotsCards, [CardType.ACTIVE])" :key="card.name"
            class="cardbox">
            <Card :card="card" />
          </div>
        </div>
    ...
    <div class="player_home_block player_home_block--setup nofloat" v-if="thisPlayer.tableau.length === 0">
      ...
      <dynamic-title v-if="playerView.pickedCorporationCard.length === 0" title="Select initial cards:"
      ...
      <div class="player_home_block player_home_block--turnorder nofloat" v-if="playerView.players.length > 1">
        ...
        <div class="player_item" v-for="(p, idx) in playerView.players" :key="idx" v-trim-whitespace>
          <div class="player_name_cont" :class="getPlayerCssForTurnOrder(p, true)">
            <span class="player_number">{{ idx+ 1 }}.</span><span class="player_name"
              :class="getPlayerCssForTurnOrder(p, false)" href="#">{{ p.name }}</span>
          </div>
      ...
      <details class="accordion board-accordion" open>
        ...
        <div class="accordion-body">
          <board :spaces="game.spaces" :venusNextExtension="game.gameOptions.venusNextExtension"
            :venusScaleLevel="game.venusScaleLevel" :boardName="game.gameOptions.boardName"
            :aresExtension="game.gameOptions.aresExtension"
            :pathfindersExpansion="game.gameOptions.pathfindersExpansion" :aresData="game.aresData"
    ...
    <div v-if="game.colonies.length > 0" class="player_home_block" ref="colonies" id="shortkey-colonies">
      ...
      <div class="colonies-fleets-cont">
        <div class="colonies-player-fleets" v-for="colonyPlayer in playerView.players" :key="colonyPlayer.color">
          <div :class="'colonies-fleet colonies-fleet-' + colonyPlayer.color"
            v-for="idx in getFleetsCountRange(colonyPlayer)" :key="idx"></div>
        </div>
      </div>
