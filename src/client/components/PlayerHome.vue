<template>
  <div id="player-home" :class="(game.turmoil ? 'with-turmoil': '')">
    ...
    <div :class="formatCssClass()" :key="componentKey">
      <div class="actions-bar-background" v-show="isExpanded()">
        <dynamic-title title="Actions" :color="thisPlayer.color"/>
        <waiting-for v-if="game.phase !== 'end'" :players="playerView.players" :playerView="playerView" :settings="settings" :waitingfor="playerView.waitingFor"></waiting-for>
      </div>
      <div class="actions-bar-collapser" v-on:click="toggleBar()">
...
export default Vue.extend({
  ...
  methods: {
    ...
    toggleBar() {
      ...
      PreferencesManager.INSTANCE.set('hide_actions_bar', this.isExpanded());
      this.forceRerender();
    },
    isExpanded(): boolean {
      return !getPreferences().hide_actions_bar;
    },
    formatCssClass(): string {
      const cssClasses = ['actions-bar', 'player_home_block', 'player_home_block--actions', 'nofloat'];
      if ( ! this.isExpanded()) {
        cssClasses.push('actions-bar-collapsed', 'player_home_block', 'player_home_block--actions', 'nofloat');
      }
